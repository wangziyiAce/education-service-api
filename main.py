...
app = FastAPI(
    ...
    version=APP_VERSION,
    debug=APP_DEBUG,
    # 所有业务接口使用 /api/v1，OpenAPI 也放在该前缀下，供前端与 Dify 文档工具统一发现。
...
def common_business_error_handler(request, exc: BusinessError):
    """把聊天与 Dify 模块的 HTTP 业务异常恢复为统一响应信封。

    ``BusinessError`` 继承 FastAPI 的 ``HTTPException``，默认会被框架包成
    ``{detail: ...}``。这里直接返回其中已定义的错误码、消息和 data 字段，确保
    前端与 Dify 调用方无需针对不同模块解析不同错误结构。
    """
...
#
# 统一由 routers.register_routers 装配已确认模块。这样 /docs、实际请求分发和
# 集成测试共享同一份路径清单；不要在本文件额外 include_router，否则容易重复注册。
# ============================================================

# 路由层只负责 HTTP 请求入口；该函数内部集中维护模块前缀和唯一认证实现。
...
if __name__ == "__main__":
    ...
    # port=8000          → HTTP 端口号
    # reload=False       → 直接执行保持单进程，避免测试与部署环境意外启动监控子进程。
    #                    开发热重载请使用文档顶部的 ``uvicorn main:app --reload`` 命令。
    uvicorn.run(
        "main:app",
        host="0.0.0.0",
        port=8000,
