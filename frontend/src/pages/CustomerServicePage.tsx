  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,.75fr)]">
      ...
      <div className="space-y-6">
        ...
        <ArchiveCard title="近期活动" index="EVENTS">{eventsQuery.isLoading && <LoadingState />}{eventsQuery.isError && <ErrorState onRetry={() => eventsQuery.refetch()} />}{eventsQuery.data?.items.length === 0 && <EmptyState icon={<CalendarDays className="h-7 w-7" />} title="暂无近期活动" description="新活动发布后会显示在这里。" />}<div className="space-y-3">{eventsQuery.data?.items.map((item) => <article key={item.id} className="min-w-0 border border-bronze/30 p-3"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><h3 className="break-words text-sm font-medium">{item.event_name}</h3><p className="mt-1 text-xs leading-5 text-muted-foreground"><CalendarDays className="mr-1 inline h-3 w-3" aria-hidden />{new Date(item.start_time).toLocaleString()} · {item.location || item.event_type}</p></div><StatusStamp label={item.status} tone="info" /></div><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" disabled={eventMutation.isPending} onClick={() => setConfirmAction({ label: `报名“${item.event_name}”`, run: () => eventMutation.mutate({ eventId: item.id, cancel: false }) })}>报名</Button><Button size="sm" variant="outline" disabled={eventMutation.isPending} onClick={() => setConfirmAction({ label: `取消“${item.event_name}”报名`, run: () => eventMutation.mutate({ eventId: item.id, cancel: true }) })}>取消报名</Button></div></article>)}</div></ArchiveCard>
      </div>
    </div>
